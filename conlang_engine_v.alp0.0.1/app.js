...
const DEFAULT_CONFIG = {
    ...
    ordemSintatica: "OVS", // Subject at the end by default, but accepts SVO
    marcaVerbo: "r", 
    cliticos: "'m, 't, 's", 
    bgColor: "#080812",
    rules: [ 
...
function importBackup(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            alert("✅ Base de dados importada e atualizada com sucesso!");
            
        } catch (err) { 
            console.error("Critical import error:", err);
            alert("❌ Erro ao ler o ficheiro JSON. A estrutura pode estar corrompida."); 
        } finally {
...
function falarPalavra(texto) {
    ...
    mapeamentos.forEach(map => {
        ...
        // Replaces orthography with IPA in temporary string for correct reading
        // Escape prevents breaking if IPA/Orth contains special regex characters
...
function renderUniversalMatrix(id) {
    ...
    groups.forEach((group, idx) => {
        ...
        activeRules.forEach(rule => {
            ...
            // MATRIX CROSS-REFERENCING LOGIC
            // Checks if rule name contains the name of ANOTHER person/class
            let belongsToOtherGroup = groups.some(g => g !== group && rule.name.includes(g));
            // Checks if rule name contains the CURRENT person/class
            let belongsToThisGroup = rule.name.includes(group);

            let finalForm = "";
            
            // If rule is exclusive to another class
            if (belongsToOtherGroup && !belongsToThisGroup) {
                finalForm = `<span style="color:var(--tx3); font-style:italic; opacity: 0.5;">-</span>`;
            } 
            // If there is a specific exception for this cell
            else if (exactMatch) {
            ...
            // If there is a general exception for this column
...
function resetDatabase() {
    const confirmacao = confirm("⚠️ WARNING: This will erase all your content! \n\n Do you want to proceed??");
...
// Interactive reader rendering logic
function renderReader() {
    const text = document.getElementById('reader-input').value;
    const output = document.getElementById('reader-output');
    
    if (!text.trim()) { 
        output.innerHTML = '<i style="color:var(--tx3); font-size: 1rem; font-family: \'Inter\', sans-serif;">No text to process...</i>'; 
        return; 
    }

    // Separates punctuation and spaces from words using Regex
    const words = text.split(/(\s+|[.,!?;:'"()]+)/).filter(Boolean);
    
    let html = words.map(token => {
        // If just punctuation or space, return as raw text
        if (/^[\s.,!?;:'"()]+$/.test(token)) {
        ...
        if (parsings.length > 0) {
            let p = parsings[0]; // Takes the first valid parsing route
        ...
        // Unknown word receives an error underline
...
const wikiPagesData = {
  phonology: "<h1>Fonologia</h1><p>Descreva o sistema sonoro do Mani aqui.</p>",
  morphology: "<h1>Morfologia</h1><p>Regras de formação de palavras, prefixos e sufixos.</p>",
  syntax: "<h1>Sintaxe</h1><p>A forma principal e mais usada tem o sujeito no final. Como alternativa para adaptação ocidental, permite-se a ordem Sujeito-Verbo-Objeto (SVO).</p>",
  alphabet: "<h1>O Alfabeto</h1><p>Caracteres especiais para documentação:</p><ul><li>რ</li><li>ฮ</li><li>L</li><li>თ</li></ul>"
};
...
function loadPage(pageId) {
  ...
  editor.innerHTML = wikiPagesData[pageId] || "<h1>Nova Página</h1><p>Comece a escrever...</p>";
}
...
function deletePage(pageId, event) {
  // Prevents click propagation to the li element
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Renders configuration settings on screen
    carregarConfiguracoesNaTela();
    
    // Renders saved dictionary words
    renderDict();
    
    // Loads the last active Wiki page
    loadPage(currentPageId);
    
    // Applies saved theme
    const savedTheme = localStorage.getItem('conlang_theme') || 'dark';
